	...
	for name, d := range eIntList {
		...
		case []interface{}:
			for _, val := range t {
				...
				if err != nil {
					return emperror.Wrap(err, "cannot create new bucket")
				}
				if err := bucket.Init(val); err != nil {
					return emperror.Wrapf(err, "cannot initialize bucket with %v", val)
				}
				fe.Buckets = append(fe.Buckets, bucket)
			}
		default:
			fmt.Errorf("invalid type %T for buckets", t)
		}
	}
	return nil
}
...
func (fa *SolrResultJSONFacets) Init(data interface{}) error {
	...
	for name, val := range eIntList {
		...
		default:
			...
			if err != nil {
				return emperror.Wrapf(err, "cannot parse facet element of %s", name)
			}
