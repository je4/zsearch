...
func encHost(host string) (string, error) {
	...
	if err != nil {
		return "", emperror.Wrapf(err, "cannot decode domain %s", host)
	}
...
func (c *Cache) BuildRefreshRSA(host string) (string, error) {
	...
	if err != nil {
		return "", emperror.Wrapf(err, "cannot  encode host %s", host)
	}
...
func (c *Cache) BuildUpdateUrl(u string, ampApiKey *rsa.PrivateKey) (string, error) {
	...
	if !ok {
		...
		if err != nil {
			return "", emperror.Wrapf(err, "cannot  encode host %s", url.Host)
		}
	...
	if err != nil {
		return "", emperror.Wrapf(err, "cannot hash path %s", updateUrl)
	}
	...
	if err != nil {
		return "", emperror.Wrapf(err, "cannog sign path %s", updateUrl)
	}
